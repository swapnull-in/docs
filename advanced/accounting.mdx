...
<Steps>
  <Step title="Register an app with the provider">
    Create a developer app in [QuickBooks Developer Portal](https://developer.intuit.com) or [Xero Developer Portal](https://developer.xero.com). Set the redirect URI to `https://billing.example.com/v1/accounting/callback/:provider`.
  </Step>
...
const connection = await recurso.accounting.connect('quickbooks');

// Response
{
  auth_url: 'https://appcenter.intuit.com/connect/oauth2?client_id=...&redirect_uri=...&state=...'
}

// Redirect the user to auth_url to authorize access
window.location.href = connection.auth_url;
```

```bash cURL
curl -X POST https://billing.example.com/v1/accounting/connect/quickbooks \
...
<CodeGroup>
```typescript TypeScript
const connections = await recurso.accounting.listConnections();
  ...
  data: [
    {
      ...
      sync_status: 'idle',
      is_active: true,
      token_expires_at: '2026-07-23T10:00:00Z',
...
```bash cURL
curl https://billing.example.com/v1/accounting/connections \
  -H "Authorization: Bearer $API_KEY"
```
</CodeGroup>
...
<CodeGroup>
```typescript TypeScript
await recurso.accounting.disconnect('acc_qb_abc123');
```

```bash cURL
curl -X DELETE https://billing.example.com/v1/accounting/connections/acc_qb_abc123 \
  -H "Authorization: Bearer $API_KEY"
```
</CodeGroup>
...
Manually trigger a sync to push all pending billing data to your connected accounting platforms.

<CodeGroup>
```typescript TypeScript
await recurso.accounting.triggerSync();

// Syncs all active connections for your tenant
// Response: 200 OK
```

```bash cURL
curl -X POST https://billing.example.com/v1/accounting/sync \
  -H "Authorization: Bearer $API_KEY"
```
</CodeGroup>

<Info>
Recurso also runs automatic syncs on a schedule. Manual sync is useful when you need data pushed immediately after a large batch of invoices or payments.
</Info>
...
<CodeGroup>
```typescript TypeScript
const status = await recurso.accounting.syncStatus();

// Response -- last 50 sync logs
{
  data: [
    {
      id: 'sync_log_001',
      connection_id: 'acc_qb_abc123',
      provider: 'quickbooks',
      status: 'completed',
      invoices_synced: 24,
      payments_synced: 18,
      credit_notes_synced: 3,
      errors: [],
      started_at: '2026-06-23T02:00:00Z',
      completed_at: '2026-06-23T02:01:45Z'
    }
    // ... up to 50 entries
  ]
}
...
```bash cURL
curl https://billing.example.com/v1/accounting/sync/status \
  -H "Authorization: Bearer $API_KEY"
```
</CodeGroup>
