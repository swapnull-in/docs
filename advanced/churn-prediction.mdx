...
```typescript
// Score: GET /v1/customers/{id}/churn  → { score: 0-100, risk_level }
const churn = (await (await fetch(
  `${BASE_URL}/v1/customers/${customerId}/churn`,
  { headers: { Authorization: `Bearer ${API_KEY}` } },
)).json()).data;

if (churn.score >= 80) {
  // Route high-risk cancellations through a retention flow session:
  // POST /v1/cancel-flows/sessions/start { customer_id, subscription_id }
  await fetch(`${BASE_URL}/v1/cancel-flows/sessions/start`, {
    method: 'POST',
