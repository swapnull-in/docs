...
// Response — the secret is only shown once
{
  ...
  environment: "live",
  prefix: "rsk_live_",
  secret: "rsk_live_7f3a9c2e1b4d8f6a0e5c3b7d9a1f4e2c8b6d0a3e5f7c9b1d4a6e8f0c2b5d7a",
  scopes: [
    "subscriptions:read",
    "subscriptions:write",
    "invoices:read",
...
```bash cURL
curl -X POST https://billing.example.com/v1/api-keys \
...
| `invoices:write` | Create and finalize invoices |
| `plans:read` | List and retrieve plans |
| `plans:write` | Create and update plans |
| `payments:read` | List and retrieve payments |
| `payments:write` | Record offline payments, issue refunds |
| `webhooks:read` | List webhook endpoints |
| `webhooks:write` | Create and update webhook endpoints |
| `usage:read` | Retrieve usage records |
| `usage:write` | Report usage events |

<Tip>
Follow the principle of least privilege. A service that only reads invoice data should not have `subscriptions:write` access. Create separate keys for separate services.
</Tip>
...
<CodeGroup>
```typescript TypeScript
await recurso.apiKeys.revoke("key_Mn4xR7kp");

// The key is immediately invalidated.
// Any request using this key will return 401 Unauthorized.
```

```bash cURL
curl -X DELETE https://billing.example.com/v1/api-keys/key_Mn4xR7kp \
  -H "Authorization: Bearer $API_KEY"
```
</CodeGroup>

<Info>
Revoking a key is permanent and takes effect immediately. All active sessions and pending requests using the revoked key will fail with a `401 Unauthorized` response.
</Info>
...
<CodeGroup>
```typescript TypeScript
const keys = await recurso.apiKeys.list({
  environment: "live"   // optional filter
});

// Response — secrets are never returned in list calls
{
  data: [
    {
      id: "key_Mn4xR7kp",
      ...
      created_at: "2025-07-10T08:30:00Z",
      last_used_at: "2025-07-15T14:22:00Z"
    }
  ]
}
```
...
```bash cURL
curl -X GET "https://billing.example.com/v1/api-keys?environment=live" \
  -H "Authorization: Bearer $API_KEY"
```
</CodeGroup>
...
```bash cURL
curl -X PUT https://billing.example.com/v1/settings \
  -H "Authorization: Bearer $API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "company_name": "Acme SaaS Pvt Ltd",
