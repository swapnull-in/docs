...
```bash cURL
curl -X POST https://billing.example.com/v1/quotes/quote_xyz/decline \
...
```bash cURL
curl -X PUT https://billing.example.com/v1/quotes/quote_xyz \
  -H "Authorization: Bearer $API_KEY" \
...
```bash cURL
curl -X DELETE https://billing.example.com/v1/quotes/quote_xyz \
  -H "Authorization: Bearer $API_KEY"
```
</CodeGroup>
