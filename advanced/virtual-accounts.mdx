...
const virtualAccount = await recurso.virtualAccounts.create({
  customer_id: "cust_8Tj3mKvR",
  invoice_id: "inv_Q4xnL92p",   // optional — ties the VA to a specific invoice
  amount: 49900                   // expected amount in paise (₹499.00)
});
...
// Response
{
  ...
  account_number: "1112109002132451",
  ifsc_code: "RATN0VAAPIS",
  bank_name: "RBL Bank",
  beneficiary_name: "Acme Corp — Invoice #Q4xnL92p",
  razorpay_va_id: "va_rzp_Di5gbNptcWV8fQ",
  status: "active",
  amount_expected: 49900,
  amount_received: 0,
  closed_at: null,
...
<CodeGroup>
```typescript TypeScript
const accounts = await recurso.virtualAccounts.list({
  customer_id: "cust_8Tj3mKvR",  // optional filter
...
```bash cURL
curl -X GET "https://billing.example.com/v1/virtual-accounts?customer_id=cust_8Tj3mKvR&status=active" \
  -H "Authorization: Bearer $API_KEY"
```
</CodeGroup>
...
<CodeGroup>
```typescript TypeScript
const payment = await recurso.payments.recordOffline({
  customer_id: "cust_8Tj3mKvR",
  invoice_id: "inv_Q4xnL92p",       // optional — link to an invoice
  payment_type: "bank_transfer",      // "bank_transfer" | "cash" | "cheque"
...
// Response
{
  ...
  notes: "Received via NEFT from HDFC account ending 4821",
  recorded_by: "ops_sanjay@acme.com",
  recorded_at: "2025-07-10T14:22:00Z"
}
```
...
```bash cURL
curl -X GET "https://billing.example.com/v1/payments/offline?customer_id=cust_8Tj3mKvR" \
  -H "Authorization: Bearer $API_KEY"
```
</CodeGroup>
