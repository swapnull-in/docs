...
```mermaid
flowchart TD
    A[Payment Fails] --> B[Build Context Key]
    B --> C[Select Algorithm]
    C --> D{Explore or Exploit?}
    D -->|Exploit| E[Pick Best Known Interval]
    D -->|Explore| F[Try Alternative Interval]
    E --> G[Schedule Retry]
    F --> G
    G --> H{Payment Result}
    H -->|Success| I[Record Reward = 1.0]
    H -->|Failure| J[Record Reward = 0.0]
    I --> K[Update Weights]
    J --> K
    K --> L[Improved Future Decisions]
```
...
```bash cURL
curl -X GET https://billing.example.com/v1/analytics/dunning/overview \
...
```bash cURL
curl -X GET "https://billing.example.com/v1/analytics/dunning/weights?context_key=USD:insufficient_funds:card:medium&limit=20" \
  -H "Authorization: Bearer $API_KEY"
```
</CodeGroup>
