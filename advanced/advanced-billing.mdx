...
```bash cURL
curl -X POST https://api.recurso.dev/v1/subscriptions/sub_xyz/charges \
...
```bash cURL
curl https://api.recurso.dev/v1/subscriptions/sub_xyz/charges \
  -H "Authorization: Bearer $API_KEY"
```
</CodeGroup>
...
```bash cURL
curl -X POST https://api.recurso.dev/v1/subscriptions/sub_xyz/advance \
  -H "Authorization: Bearer $API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "periods": 6
