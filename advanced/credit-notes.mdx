...
```bash cURL
curl -G https://billing.example.com/v1/credit-notes \
...
<CodeGroup>
```typescript TypeScript
await recurso.creditNotes.void('cn_xyz');
```

```bash cURL
curl -X POST https://billing.example.com/v1/credit-notes/cn_xyz/void \
  -H "Authorization: Bearer $API_KEY"
```
</CodeGroup>
