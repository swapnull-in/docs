...
```typescript
// The 24-hour RBI pre-debit notice is sent automatically by the built-in
// pre-charge scheduler for every UPI AutoPay mandate — there is nothing to
// enable. It fires ~24h before each scheduled debit; see the mandate lifecycle
// under UPI AutoPay.
```
...
```typescript
// Consent history for a customer:
//   GET /v1/customers/{id}/consents
const res = await fetch(`${BASE_URL}/v1/customers/cust_abc/consents`, {
  headers: { Authorization: `Bearer ${API_KEY}` },
});
const { data: consents } = await res.json();
```
...
```typescript
// Each consent record carries its own grant/revoke trail. Records are
// created with POST /v1/consents and withdrawn with POST /v1/consents/revoke;
// the full history reads back from GET /v1/customers/{id}/consents. The
// append-only audit log (POST/PUT/DELETE across the API) captures the actor,
// IP, and timestamp of every change — see /advanced/webhooks and the audit
