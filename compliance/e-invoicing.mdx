...
```bash cURL
curl -X POST https://api.recurso.dev/v1/invoices/inv_001/einvoice/retry \
  -H "Authorization: Bearer $API_KEY"
```
</CodeGroup>
...
```bash cURL
curl -X POST https://api.recurso.dev/v1/invoices/inv_001/einvoice/cancel \
  -H "Authorization: Bearer $API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "cancel_code": 1,
...
| `einvoice.void` | E-invoice was cancelled on the IRP |
