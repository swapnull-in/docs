...
<Note>
The examples below use `https://billing.example.com` as a placeholder for your Recurso instance. Substitute the domain where your deployment is hosted.
</Note>

```bash
curl -X POST https://billing.example.com/v1/webhooks \
...
```bash
curl -X POST https://billing.example.com/v1/einvoice/irp-config \
  -H "Authorization: Bearer $API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X POST https://billing.example.com/v1/einvoice/test-irp \
  -H "Authorization: Bearer $API_KEY"
```
...
[SEV1 incident runbook](https://github.com/swapnull-in/recur-so/blob/main/docs/incident-runbook.md)
