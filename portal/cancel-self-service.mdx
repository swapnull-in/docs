...
This feature integrates directly with the [Cancel Flows API](/advanced/cancel-flows) to render your configured surveys, retention offers, and confirmation steps inside the portal experience.
...
```bash cURL
curl -X POST https://billing.example.com/v1/portal/sessions \
...
```bash cURL
curl -X PUT https://billing.example.com/v1/settings/portal \
  -H "Authorization: Bearer $API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "self_service": {
...
The portal cancel experience is driven entirely by your Cancel Flows configuration. Any changes you make via the [Cancel Flows API](/advanced/cancel-flows) are immediately reflected in the portal.
